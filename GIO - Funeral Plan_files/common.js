...
f().join("")&&E(this,f(),b().p);a.addClass("focus.inputmask");b().undoBuffer=this._valueGet();a.click()}).bind("mouseleave.inputmask",function(){var a=c(this);h.clearMaskOnLostFocus&&(a.hasClass("focus.inputmask")||(this._valueGet()==g().join("")||""==this._valueGet()?this._valueSet(""):T(this)))}).bind("click.inputmask touchend.inputmask",function(){var a=this;setTimeout(function(){var d=v(a),e=f();if(d.begin==d.end){var d=d.begin,g=b().lastValidPosition,g=h.isNumeric?!1===h.skipRadixDance&&""!=h.radixPoint&&-1!=c.inArray(h.radixPoint,
...
var directLifeModule = directLifeModule || angular.module("DirectLife", []);

directLifeModule.directive('dlCalculate', function() {
    return {
        priority: 0,
...
directLifeModule.directive("dlGlobalErrorBroadcaster", function () {
    var broadcastEvent = function (scope, element) {
        var inputs = $(element).find("input:radio");
        jQuery.each(inputs, function () {
            jQuery(this).on('click', function () {
                scope.$broadcast("toggleGlobalError");
            });
        });
    };
...
directLifeModule.directive("dlGlobalErrorListener", function () {

    var hasDateOfBirthError = function (element) {
        var dateOfBirthErrorMessageText = jQuery(element).find('#globalError0').text();
        var hasError = false;
        if (dateOfBirthErrorMessageText.indexOf("between 18 and 75 years") !== -1) {
            hasError = true;
        }
        return hasError;
    };

    var shouldShowGlobalErrors = function (element) {
        var errorElements = jQuery(element).parent().find('div[id*=".error"]');
        ...
        return  hasDateOfBirthError(element) || hasFieldError;
    };

    var listenToEvent = function (scope, element) {
        scope.$on('toggleGlobalError', function (event, msg) {
            scope.shouldShowGlobalError = shouldShowGlobalErrors(element);
            if (!scope.$$phase) {
                scope.$apply();
            }
        });
...
directLifeModule.directive('dlGrayDefaultSelect', function () {
    return {
        restrict: 'C',
        link: function (scope, elm) {
            var grayTextClassName = 'gray-text';
            elm.addClass(grayTextClassName);
            elm.bind('change', function () {
                if (elm.val() === "") {
                    elm.addClass(grayTextClassName);
                } else {
                    elm.removeClass(grayTextClassName);
                }
            });
        }
    };
...
directLifeModule.directive('dlHoverShading', function () {
    return function (scope, element, attrs) {
        var colorClass = attrs.dlHoverShading || 'hover-background-color';
        jQuery(element).hover(
            function () {
                jQuery(this).addClass(colorClass);
            },
            function () {
                jQuery(this).removeClass(colorClass);
            }
        );
    };
});
...
directLifeModule.directive('dlMaskName', function () {
    return {
        link: function ($scope, element) {
            return jQuery(element).alpha({allow: "'-"});
        }
    };
});

directLifeModule.directive('dlMaskNumber', function () {
    return {
        link: function ($scope, element) {
            return jQuery(element).alphanum({allowLatin: false, allowOtherCharSets: false, allow:"\r"});
        }
    };
});

directLifeModule.directive('dlMaskBsb', function () {
    return {
        link: function ($scope, element) {
            return jQuery(element).alphanum({allowLatin: false, allowOtherCharSets: false, allowSpace: false, allow:"\r"});
        }
    };
});

directLifeModule.directive('dlMaskAngular', function() {
    return {
        require: 'ngModel',
        link: function($scope, element, attrs, controller) {
            controller.$render = function() {
                jQuery(element).inputmask(attrs.dlMaskAngular);
            };
        }
    };
});
...
directLifeModule.directive('dlModelOnblur', function() {
    return {
        restrict: 'C',
        require: 'ngModel',
        link: function(scope, elm, attr) {
            var func = function() {
                scope.$apply(function() {
                    scope.$eval(attr.ngModel + " = '"+ document.getElementById(attr.id).value +"'");
                });
            };
            ...
            elm.unbind();
            jQuery(elm).on('blur focusout', func);
            if (jQuery(elm).prop("tagName") === 'SELECT') {
                jQuery(elm).on('click', func);
            }
        }
    };
});
...
directLifeModule.directive('dlNavigation', function() {
    return {
        restrict: 'A',
        scope: {status: '@', navigationId: '@'},
        link: function($scope, $elm, $attr) {
            if ($attr.status === "previous") {
                jQuery($elm).on("click", function() {
                    jQuery("#" + $attr.navigationId).val("true");
                    jQuery('form').submit();
                });
            }
        }
...
directLifeModule.directive("dlPaymentMethodToggle", function(){
    return {
        restrict: 'A',
        link: function (scope, element) {
            var inputs = $(element).find("input:radio");
            jQuery.each(inputs, function(){
                jQuery(this).on('click', function(){
                    scope.paymentMethod = jQuery(this).val();
                    scope.$apply();
                });
            });
        }
...
var DirectLife = DirectLife || {};

DirectLife.pieAttachment = function () {
    if (window.PIE) {
        $('.attach-pie').each(function() {
            PIE.attach(this);
        });
    }
};
...
DirectLife.initializePlaceholder = function () {
...
directLifeModule.directive('dlTooltip', function () {
    return function (scope, element, attrs) {
        if ($(window).width() <= 600) {
            var question = jQuery(element).find('.tooltip-image');
            jQuery(element).popover({
                animation: false,
                html: true,
                placement: 'right',
                trigger: 'manual',
                content: attrs.dlTooltip
            });
            ...
            jQuery(element).on('hidden.bs.popover', function(){
                jQuery(question).click(function(){
                    jQuery(element).popover('show');
                    return false;
                });
...
directLifeModule.directive("dlPostcodeLookup", function ($http) {
    return {
        restrict: 'AC',
        link: function (scope, elm) {
            function onSelect($target, selectFn, event, ui) {
                function moveCaretToFront(textBox) {
                    if (textBox.createTextRange) {//IE
                        var range = textBox.createTextRange();
                        range.collapse(true);
                        range.select();
                    } else if (textBox.setSelectionRange) {// Chrome
                        textBox.setSelectionRange(0, 0);
                    }
                }

                $target.val(ui.item.value !== undefined ? ui.item.value : ui.item.label);
                moveCaretToFront($target[0]);
            ...
            function onAjaxSuccess(response, data) {
            ...
            function onSource($target, settings, request, response) {
                $target.addClass("loading");
                var trimmedValue = $.trim(request.term);
                if (trimmedValue.length > settings.minLength - 1) {
                    $http({
                        url: '/directlife/postcodeSuburb',
                        method: "GET",
                        params: { "postcodeSuburbValue": trimmedValue }
                    }).success(function (data) {
                        $target.removeClass("loading");
                        settings.onAjaxSuccess(response, data);
                    });
                }
            }

            function autoCompletePostcode($target, settings) {
                var defaultSettings = {
                    minLength: 3,
                ...
                var settingsToUse = $.extend(defaultSettings, settings);
                settingsToUse.source = onSource.bind(this, $target, settingsToUse);
                settingsToUse.select = onSelect.bind(this, $target, settingsToUse.select);

                return $target.autocomplete(settingsToUse);
            }

            $.fn.postcodeAutocomplete = function (/* settings */) {
                return autoCompletePostcode(this, arguments[0] || {});
            };

            var $postCode = $(elm);
            $postCode.postcodeAutocomplete();
        }
    };
});
/*global angular:true*/
...
directLifeModule.directive("dlPreventDefault", function(){
    var preventDefault = function(scope, element) {
...
DirectLife.QuoteController = function ($rootScope, $scope, $http, redirectService) {
    $scope.quoteCriteria = {};
    $scope.premiumDetails = {};
    $scope.errors = {};
    $scope.isLoading = false;
    $scope.formDirty = false;
    $scope.shouldShowGlobalError = true;

    $scope.init = function (quoteContainer) {
        $scope.quoteCriteria = quoteContainer.quoteDetails;
        $scope.errors = quoteContainer.errors;
        $scope.premiumDetails = $scope.quoteCriteria.applicantDetails.premiumAmount;
        $scope.$watch('quoteCriteria', function (newValue, oldValue) {
            if ($scope.shouldShowPremium() && !angular.equals(newValue, oldValue)) {
    ...
    $scope.calculate = function () {
        ...
        $http({
            url: 'price/estimate',
            method: "POST",
            data: $scope.quoteCriteria
        }).success(function (data, status) {
                if (status === 200 && data.quoteDetails === undefined) {
                    redirectToOtherPage(data);
                } else {
                    $scope.premiumDetails = data.quoteDetails.applicantDetails.premiumAmount;
    ...
    $scope.shouldShowPremium = function () {
        return greaterThanZero($scope.premiumDetails) && !$scope.objectContainsElements($scope.errors.global);
    };

    $scope.objectContainsElements = function (objectToCheck) {
        var length = 0;
    ...
    $scope.showGlobalError = function (objectToCheck) {
        return $scope.objectContainsElements(objectToCheck) && $scope.shouldShowGlobalError;
    };

    $scope.hasFamilyDiscount = function () {
        return $scope.quoteCriteria.familyMemberDetails.hasFamilyDiscount === 'Yes';
    };

    $scope.cleanFamilyDiscount = function () {
        if ($scope.quoteCriteria.familyMemberDetails.hasFamilyDiscount === 'No') {
            $scope.quoteCriteria.familyMemberDetails.familyMemberPolicyNumber = '';
            $scope.quoteCriteria.familyMemberDetails.familyMemberFirstName = '';
            $scope.quoteCriteria.familyMemberDetails.familyMemberSurname = '';
            $scope.quoteCriteria.familyMemberDetails.familyMemberDateOfBirth = '';
            $scope.errors.fields['familyMemberDetails.familyMemberPolicyNumber'] = '';
            $scope.errors.fields['familyMemberDetails.familyMemberFirstName'] = '';
            $scope.errors.fields['familyMemberDetails.familyMemberSurname'] = '';
            $scope.errors.fields['familyMemberDetails.familyMemberDateOfBirth'] = '';
        }
    };

    var redirectToOtherPage = function (data) {
        if (data.search('Technical Error') !== -1) {
            redirectService.doRedirect("/directlife/error");
        } else {
            redirectService.doRedirect("/directlife/timeout");
        }
    };

    var greaterThanZero = function (valueToCheck) {
        return (valueToCheck > 0);
    };

    var toggleLoading = function (isLoading) {
        $scope.isLoading = isLoading;
        $scope.$broadcast("toggleSpinner", {"isLoading": isLoading});
    };

    var toggleGlobalError = function () {
        // send broadcast after page view rendered to toggle global error
        setTimeout(function () {
            if(!$scope.$$phase){
                $scope.$apply(function(){
                    $scope.$broadcast("toggleGlobalError");
...
directLifeModule.controller('QuoteController', DirectLife.QuoteController, [ '$rootScope', '$scope', '$http', 'redirectService']);

/*global angular:true*/

var directLifeModule = directLifeModule || angular.module("DirectLife", []);

directLifeModule.directive("dlRadioButton", function(){
    var removeClass = function(parentElement) {
        var labels = jQuery(parentElement).find("label");
        $.each(labels, function(){
            jQuery(this).removeClass("active");
        });
    };

    var addClass = function(element, id) {
        jQuery(element).find("label[for='" + id + "']").addClass("active");
    };

    var bindOnChange = function(scope, element) {
        var inputs = $(element).find("input:radio");
...
directLifeModule.factory("redirectService", ["$window", function ($window) {
    return {
        doRedirect: function (url) {
            var port = $window.location.port ? ":" + $window.location.port : "";
            $window.location.href = $window.location.protocol + "//" + $window.location.hostname + port + url;
        }
    };
}]);
/*global angular:true*/
...
directLifeModule.directive("dlSpinner", function(){
    var bindSpinner = function(scope, element) {
        var bar_length = 5, bar_width = 5, inner_radius = 8;
        var width = parseInt(jQuery(element).width(), 10);
        var height = parseInt(jQuery(element).height(), 10);

        var opts = {
            length: bar_length, // The length of each line
            width: bar_width, // The line thickness
            radius: inner_radius, // The radius of the inner circle
            color: '#000000', // The spinner color
        ...
        function positionRelative(element, left, top) {
            jQuery(element).css('position', 'relative');
            jQuery(element).css('left', left + 'px');
            jQuery(element).css('top', top + 'px');
        }

        scope.$on('toggleSpinner', function (event, msg) {
            if(msg.isLoading) {
                jQuery(element).button('loading');
                spinner.spin();
                jQuery(element).append(spinner.el);
                positionRelative(spinner.el, opts.left, opts.top);
            } else {
...
directLifeModule.directive("dlSpinnerToggle", function(){
    return function(scope, element) {
        var mutex = false;
        jQuery(element).on('click', function(){
            scope.$broadcast('toggleSpinner', {isLoading: true});
            if (!mutex) {
                mutex = true;
                jQuery("form").submit();
            }
...
directLifeModule.directive("dlStartApplication", function(){
    var preventPropagation = function(scope, element) {
        jQuery(element).click(function(event){
            event.preventDefault();
            scope.formDirty = false;
            jQuery(element).focus();
...
/*global angular:true BaseQuestion*/

var directLifeModule = directLifeModule || angular.module("DirectLife", []);

directLifeModule.factory('questionnaireService', ['$http', function ($http) {
    return {
        findDisclosures: function(questions, identifier){
            for (var index in questions){
                if (questions[index].id === identifier){
                    return questions[index].linkedDisclosures;
                }
            }
            return [];
        },
        registerClickForYes: function(questions) {
            var self = this;
            $('span.option.yes').change(function(){
                var elem = this;
                $(elem).siblings('div').remove();

                var identifier = $(this).find('>input').attr('name').slice(4);
                var linkedDisclosures = self.findDisclosures(questions, identifier);

                var newSection = $("<div class='answers'></div>");
                $(linkedDisclosures).each(function(index, disclosure){
                    var identifier = 'ure' + disclosure.id;
                    newSection.append("<span class='ure-disclosure'><input type='checkbox' name='"+ identifier +"'>" + disclosure.name + "</span>");
                });

                $(elem).closest('.ure-basequestion').append(newSection);
            });
        },
        renderBaseQuestions: function () {
            var self = this;
            $http({
                url: 'questionnaire',
                method: 'GET'
                ...
                .success(function (questionnaire) {
                    ...
                    container.html("");
                    $(questionnaire.questions).each(function (index, question) {
                        var baseQuestion = new BaseQuestion(question);
                        container.append(baseQuestion.toHtml());
                    });
                    self.registerClickForYes(questionnaire.questions);
                })
                .error(function () {
        ...
        load: function () {
            $(document).ready($.proxy(this.renderBaseQuestions(), this));
        }
    };
}]);

/*global angular:true ReflexiveQuestion*/

var directLifeModule = directLifeModule || angular.module("DirectLife", []);

directLifeModule.factory('reflexiveService', ['$http', function ($http) {
    return {
        loadReflexiveQuestions: function (event) {
            ...
            $http({
                ...
                .success(function (reflexiveQuestions) {
                    var disclosureContainer = element.closest('.ure-disclosure');
                    var question = new ReflexiveQuestion(reflexiveQuestions.rootQuestion);
                    question.renderIn(disclosureContainer);
                })
                .error(function (e) {
                    // how to handle this error?
                });
        ...
        load: function () {
            jQuery('#questionnaire').on('click', '.ure-basequestion>.answers>span', $.proxy(this.loadReflexiveQuestions, this));
        }
    };
}]);
...
DirectLife.UnderwritingController = function ($scope, $http, reflexiveService, questionnaireService) {

    $scope.init = function () {
        questionnaireService.load();
        reflexiveService.load();
    };
};

directLifeModule.controller('UnderwritingController', DirectLife.UnderwritingController, ['$scope', '$http', 'reflexiveService']);

var BaseQuestion = function(questionAsJson) {
    this.baseQuestion = questionAsJson;
};

BaseQuestion.prototype.toHtml = function() {
    var baseQuestion = $("<div class='ure-basequestion'></div>");
    var identifier = "ure-" + this.baseQuestion.id;
    var labelElement = $("<label>" + this.baseQuestion.description + "</label>");
    labelElement.addClass("question");
    var isHeadingQuestion = ("HEADING" === this.baseQuestion.type);
    if (isHeadingQuestion) {
        labelElement.addClass("headingQuestion");
    }
    baseQuestion.append(labelElement);
    if (!isHeadingQuestion) {
        baseQuestion.append("<span class='option yes'><input type='radio' name='" + identifier + "' value='Yes' />Yes</span>");
        baseQuestion.append("<span class='option'><input type='radio' name='" + identifier + "' value='Yes' />No</span>");
    }
    return baseQuestion;
};
var ReflexiveQuestion = function (questionAsJson) {
    this.questionAsJson = questionAsJson;
};

ReflexiveQuestion.prototype.toHtml = function () {
    var htmlElement = $("<div class='ure-reflexivequestion'></div>");
    htmlElement.append("<label>" + this.questionAsJson.description + "</label>");
    htmlElement.append(this.optionsToHtml());
    return htmlElement;
};

ReflexiveQuestion.prototype.renderIn = function (disclosureContainer) {
    disclosureContainer.find('.ure-reflexivequestion').remove();
    disclosureContainer.append(this.toHtml());
};

ReflexiveQuestion.prototype.options = function () {
    return this.questionAsJson.nextLevelQuestions;
};

ReflexiveQuestion.prototype.optionsToHtml = function () {
    var htmlElement = $("<div></div>");
    $(this.options()).each(function(index, option) {
        var questionId = "ure-reflexivequestion-" + option.id;
        htmlElement.append("<input id='" + questionId + "' type='checkbox' value='" + option.branchValue + "'/>");
        htmlElement.append("<label for='" + questionId + "'>" +  option.branchText + "</label>");
    });
    return htmlElement;
};
